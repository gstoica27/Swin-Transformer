...
def main(config, logger):
    ...
    if config.TRAIN.BISA_LAMBDA_REGIME[1] > config.TRAIN.START_EPOCH:
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if config.TRAIN.BISA_LAMBDA_REGIME[1] > 0 and epoch >= config.TRAIN.BISA_LAMBDA_REGIME[1] and not flag:
            logger.info('Training Lambda Now!')
            flag = False
            for name, parameter in model_without_ddp.named_parameters():
                if 'selection_lambda' in name:
        ...
        if acc1 > max_valid_accuracy:
            prev_max_accuracy = max_valid_accuracy
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, after_ffcv_transform, lr_scheduler, scaler, logger):
    ...
    for idx, (samples, targets, samples2, orig_imgs) in enumerate(data_loader):
        ...
        if not config.FFCV:
            ...
            samples2 = samples2.cuda(non_blocking=True)

        if mixup_fn is not None:
            samples, samples2, targets = mixup_fn(samples, samples2, targets)
        else:
            targets = torch.nn.functional.one_hot(targets, num_classes=config.MODEL.NUM_CLASSES)

        if config.CROSS.APPLY_EDGE:
            samples2 = K.filters.sobel(samples)
        
        with autocast(enabled=config.NATIVE_AMP):
            # all_samples = torch.cat((samples, samples2), dim=0)
            # all_targets = torch.cat((targets, targets), dim=0)
            outputs, outputs2 = model(
                # all_samples,
                # all_targets,
                samples,
                samples2,
            ...
            loss = (criterion(outputs, targets) + criterion(outputs2, targets)) / 2
...
def validate(config, data_loader, model, logger):
    ...
    for idx, (images, target, _, _) in enumerate(data_loader):
        images = images.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)

        # compute output
        with autocast(enabled=config.NATIVE_AMP):
            output, _ = model(images, use_amp=False)#, apply_edge=False)
