...
from models.csam import ConvolutionalSelfAttention
...
class CSAMInjSwinTransformer(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, approach_args=None, **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            if approach_args is not None and i_layer in self.approach_args.INJECTION_LOCATIONS:
                print('ADDING CSAM!!!!')
                layer = ConvolutionalSelfAttention(
                    spatial_shape=[
                        patches_resolution[0] // (2 ** i_layer),
                        patches_resolution[1] // (2 ** i_layer),
                        int(embed_dim * 2 ** i_layer)
                    ],
                    filter_size=self.approach_args.FILTER_SIZE,
                    approach_args=self.approach_args
                )
