...
class BiDirectionalWindowAttention(nn.Module):
    ...
    def apply_inverse_attention(self, x, mask=None):
        BW, K2, C = x.shape
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(BW, K2, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        ...
        global_inputs = self.activation(self.global_proj(x).reshape(BW, N, self.num_heads, self.embed_dim).transpose(-3, -2))
        local_inputs = self.activation(self.local_proj(x).reshape(BW, N, self.num_heads, self.embed_dim).transpose(-3, -2))

        global_summaries = (attn @ global_inputs)

        global_weights = self.G(global_inputs).reshape(BW, self.num_heads, N, self.embed_dim, self.embed_dim)
    ...
    def forward(self, x, mask=None):
        # pdb.set_trace()
        if self.is_bidirectional:
            x = self.apply_inverse_attention(x, mask=mask)
        output = self.apply_forward_attention(x, mask=mask)
