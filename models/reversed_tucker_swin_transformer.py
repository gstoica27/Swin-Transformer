...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.relative_position_bias_table_no_head = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), 1))  # 2*Wh-1 * 2*Ww-1, nH
        self.attention_parameters.extend([self.relative_position_bias_table, self.relative_position_bias_table_no_head])
    ...
    def instantiate_generator_weights(self):
        if 'reverse' in self.mechanism:
            self.W = nn.Parameter(torch.rand((self.embed_dim, self.embed_dim, self.embed_dim)), requires_grad=True)
            self.V = nn.Parameter(torch.rand((self.dim, self.embed_dim)), requires_grad=True)
    ...
    def apply_reverse_attention(self, x, attn):
        ...
        Vi = x @ self.V                                   # [B, HW, D]
        Jj = expert_mixture @ self.J                      # [B, HW, D]
        W_Jj = torch.einsum('BKD,DEF->BKEF', Jj, self.W)  # [B, HW, D, D]
        W_JjVi = torch.einsum('BKD,BKDE->BKE', Vi, W_Jj)  # [B, HW, D]
        output = W_JjVi @ self.U                               # [B, HW, C]
        
        return output

    def forward(self, x, mask=None):
        if self.mechanism == 'forward':
            attn = self.compute_attn(x, mask)
            output = self.apply_forward_attention(x, attn)
        elif self.mechanism == 'reverse':
            attn = self.compute_attn(x, mask, multiheaded=False)
    ...
    def compute_attn(self, x, mask, multiheaded=True):
        num_heads = self.num_heads if multiheaded else 1

        BW, K2, C = x.shape
        qk = self.qk(x).reshape(BW, K2, 2, num_heads, C // num_heads).permute(2, 0, 3, 1, 4)
        ...
        if multiheaded:
            ...
            relative_position_bias = self.relative_position_bias_table_no_head[self.relative_position_index.view(-1)].view(
                self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(BW // nW, nW, num_heads, K2, K2) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, num_heads, K2, K2)
